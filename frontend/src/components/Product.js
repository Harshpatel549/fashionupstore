...
function Product(props) {
  ...
  return (
    <Card style={{border:'none', padding:'10px', minHeight:'619px', maxHeight:'619px', overflow:'hidden' , maxWidth:'320px', minWidth:'320px'}}>
      <Link to={`/product/${product.slug}`}>
        <img src={product.image} style={{maxWidth:'320px', minWidth:'320px'}} className="card-img-top" alt={product.name} />
      </Link>
      <Card.Body>
        <Link to={`/product/${product.slug}`}>
          <Card.Title className='productTitleName'>{product.name}</Card.Title>
        </Link>
